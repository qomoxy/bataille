...
class Carte:
    ...
    def __eq__(self, carte2):
        ...
        return self.get_point() == carte2.get_point()
    ...
    def tete(self):
        ...
        :return : la tête de la carte ou sont nombre de points
        """
        match self.__point:
            case 11:
    ...
    def __str__(self):
        """Affiche la carte."""
        if self.tete() in self.get_tete():
            return f"La carte à tête : {self.tete()} et pour symbol : {self.get_symbol()}"
        else:
...
class Deck:
    ...
    def ajouter(self, carte):
        ...
        :param carte: la carte à rajouter
        """
    ...
    def partage(self):
        ...
        :return : paquet1 (file), paquet2(file)
        """
        paquet1 = File()
        paquet2 = File()
        for i in range(len(self.__deck)):
            if i % 2 == 0:
                paquet1.enfile(self.__deck[i])
            else:
                paquet2.enfile(self.__deck[i])
